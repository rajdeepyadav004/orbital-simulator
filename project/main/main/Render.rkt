...
(struct project( x y r) #:transparent)
(define (planet-projection planet reference normal distance-from-screen)
  ...
  (define (centre-radius); refrence is a class vector%, normal is struct v(a b c), it is also unit vector
    (let* ([V0 reference]
           ...
           [k (if (= (+ (* a a1) (* b b1) (* c c1)) 0) 0 (/ D (+ (* a a1) (* b b1) (* c c1))))]
           [V2 (add-vector V0 (send direction-ratio scalar-mult k))]
           [d (send (sub-vector V1 V0) magnitude)]
           [r (send planet show-radius)]
           [radius (if (= d 0) r (/ (* r D) d))])
      (cons V2 radius)))
...
;(define Sun (make-object planetary-object% 100 100 (make-vector% 100 0 0) (make-vector% 100 0 0 ) (make-vector% 100 0 0 ) "red"))
